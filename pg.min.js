    ...
    try {
      ...
      if (isManifest(url)) {
        ...
        return Promise.race([
          ...
          new Promise(function (_, rej) { setTimeout(function () { rej(new Error("manifest timeout")); }, 2500); })
        ]).catch(function () {
          return new Response(
...
(function () {
  ...
  ready(function(){
    // Paksa semua ancestor yang menyembunyikan container jadi terlihat
    function unhideChain(id, h){
      var $box = $(id);
      if(!$box.length) return;
      $box.parents().each(function(){
        var $t = $(this);
        if ($t.css('display') === 'none') $t.css('display','block');
        if ($t.css('visibility') === 'hidden') $t.css('visibility','visible');
      });
      $box.css({opacity:1, height:h});
    }

    // Coba beberapa kali (buat case event postMessage telat)
    var tries = 0, timer = setInterval(function(){
      unhideChain('#qriscontainer','350px');
      unhideChain('#toppaycontainer','400px');
      // Paksa select re-evaluate supaya eventSelect nge-show template
